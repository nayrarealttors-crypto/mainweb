...
export default function EnquiryModal({ slide, onClose }) {
  const [form,    setForm]    = useState(INITIAL);
  const [status,  setStatus]  = useState('idle');
  const [message, setMessage] = useState('');

  useEffect(() => {
    if (slide) { setForm(INITIAL); setStatus('idle'); setMessage(''); }
  }, [slide]);
  ...
  useEffect(() => {
    ...
    if (slide) document.body.style.overflow = 'hidden';
    return () => {
          ...
          >
            ...
            >
              <div className="p-6 md:p-8">
                ...
                <p className="eyebrow mb-2">Property Enquiry</p>
                ...
                {status === 'success' ? (
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                    ...
                    <input
                      ...
                      onFocus={focusIn} onBlur={focusOut}
                    />
                    {/* Project Name — read-only, pre-filled */}
                    <input
                      readOnly value={slide.title}
                      className={inputCls}
                      style={{ ...inputStyle, background: '#fdfaf5', cursor: 'not-allowed', color: '#7a8a7e' }}
                    />
